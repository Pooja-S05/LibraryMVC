...
namespace LibraryMSMVC.Controllers
{
    public class BorrowController : Controller
    {
        ...
        public ActionResult Index(int? userId, string userName)
        {
            ...
            tblUser user = tblUserdb.tblUsers.Find(userId);
            if (user == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Borrow(int? bookId)
        {
            ...
            if (tblUserdb.tblTransactions.Where(t => t.UserId == userId).Count() < 6)
            {
                if (bookId != null)
                {
                    tblBook book = tblUserdb.tblBooks.FirstOrDefault(b => b.BookId == bookId);
                    if (book == null)
                    {
                        return HttpNotFound();
                    }
                    if (book.BookCopies > 0)
                    {
                        ...
                        tblUserdb.tblTransactions.Add(trans);
